...
# from schemes import InitLoginRequest, Profile, Card_Data, InitAuthResponse, Card_Data_With_Attempt
...
# REDIS_SESSION_ID_PREFIX = "session:"     # s_id -> uuid 매핑 키 (s_id 자체를 키로 사용)
# REDIS_UUID_TO_SESSION_PREFIX = "uuid_session:" # uuid -> s_id 역매핑 키
...
# @ospass_router.post("/v1/nfc-init-auth", response_model=InitAuthResponse)
# def init_nfc_authentication(APIKEY:str=Query(..., description="서비스 식별 APIKEY"),
#                             redirect_uri:str=Query(...,description="인증 완료 후 리다이렉트 될 서비스 URI"),
#                             state:Optional[str]=Query(None, description="Oauth 2.0 state parameter"),
#                             db:Session=Depends(get_db)):
#     """
#     - 서비스 서버가 호출: NFC 인증 시작을 요청하고 시도 ID(attempt id) 발급
#     :params
#     - APIKEY: 사용자 서비스의 APIKEY
#     - redirect_uri: 사용자 서비스의 Redirect URI
#     - state: 서비스 서버가 요청에 대한 응답을 연결하기 위한 문자열(Oauth 2.0 표준)
#     :retruns
#     - attempt_id: 이번 NFC 인증 시도 고유 식별자
#     """
#     try:
#         # STEP 1. APIKEY와 redirect_uri 유효성 검증
...
#         if not api_key_row:
#             logger.warning(f"NFC 인증 시작 실패: Invalid APIKEY:{APIKEY}")
#             raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
#                                 detail="Invalid APIKEY")
#         # Redirect URI 등록 여부 판단 
#         registered_services = api_key_row.registered_service 
#         redirect_uri_is_registered = False
#         if isinstance(registered_services, dict):
#             for service_info in registered_services.values():
#                 if isinstance(service_info, dict) and service_info.get("redirect_uri") == redirect_uri:
#                     redirect_uri_is_registered = True
#                     break
#         if not redirect_uri_is_registered:
#             logger.warning(f"NFC 인증 시작 실패: redirect_uri{redirect_uri} not found")
#             raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
#                                 detail="Redirect URI not found")
        
#         # STEP 2. 고유 시도 ID(attempt_id) 생성
#         attempt_id = str(uuid.uuid4())
        
#         # STEP 3. Redis에 시도 상태 저장(pending)
#         # attempt_id: APIKEY, redirect_uri, state, 초기 상태(pending) 및 TTL 저장
#         attempt_state = {
#             "status" : "pending",
#             "APIKEY" : APIKEY,
#             "redirect_uri" : redirect_uri,
#             "state" : state,
#             "s_id" : None # s_id: NFC 인증 완료 시 추가 예정(식별 세션 ID)
#         }
#         # 시도 유효 시간(5분)
#         attempt_ttl_sec = 300 #300초
#         redis_key = f"{REDIS_AUTH_ATTEMPT_PREFIX}{attempt_id}"
#         rd.setex(redis_key, attempt_ttl_sec, json.dumps(attempt_state))
        
#         logger.info(f"NFC 인증 시도 시작, attempt_id:{attempt_id}")
        
#         return InitAuthResponse(attempt_id=attempt_id)
#     except HTTPException as he:
#         raise he
#     except Exception as e:
#         logger.error(f"Unexpected Error Occured in Initiate NFC Auth:{str(e)}")
#         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
#                             detail="Faield to initiate NFC Auth")
...
#     '''
#     - OSTOOLS에서 호출될 API
#     - attempt_id와 NFC 데이터를 받아, 해당 시도를 성공 상태로 업데이트 하고 사용자 연결
#     '''
#     attempt_id = data.attempt_id
#     try:
#         logger.debug(f"/v1/card-response received for attempt_id: {attempt_id}, client_id: {client_id}")
#         redist_attempt_key = f"{REDIS_AUTH_ATTEMPT_PREFIX}{attempt_id}"
#         attempt_state_bytes = rd.get(redist_attempt_key)
        
#         if not attempt_state_bytes:
#             logger.warning(f"/v1/card-response failed: Invalid or expired attempt_id: {attempt_id}")
#             raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
#                                 detail="Invalid or expired authenticate attempt")
...
# @ospass_router.get("/v1/profile", response_model=Profile)
